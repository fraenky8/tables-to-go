...
CREATE TABLE char_table (
    ...
    char_nn_check_cmp char NOT NULL CHECK ( char_nn_check_cmp = '42' ),
    char_nn_check_fn char NOT NULL CHECK ( length(char_nn_check_fn) > 0 ),
    ...
    char_nn_unique_check char NOT NULL UNIQUE CHECK ( length(char_nn_unique_check) > 0 ),

    char_unique char UNIQUE,
    char_unique_check char UNIQUE CHECK ( length(char_unique_check) > 0 ),
    ...
    char_check char CHECK ( length(char_check) > 0 ),
    char_check_ref char CHECK ( length(char_check_ref) > 0 ) REFERENCES char_ref(char_ref),
    char_check_def_const char CHECK ( length(char_check_def_const) > 0 ) DEFAULT ('42'),
    char_check_def_func char CHECK ( length(char_check_def_func) > 0 ) DEFAULT (pi()),

    char_ref char REFERENCES char_ref(char_ref),
    char_ref_unique_check char UNIQUE CHECK ( length(char_ref_unique_check) > 0 ) REFERENCES char_ref(char_ref),

    char_def_const char DEFAULT ('42'),
    char_def_const_unique_check char UNIQUE CHECK ( length(char_def_const_unique_check) > 0 ) DEFAULT ('42'),

    char_def_func char DEFAULT (pi()),
    char_def_func_unique_check char UNIQUE CHECK ( length(char_def_func_unique_check) > 0 ) DEFAULT (pi())
);
