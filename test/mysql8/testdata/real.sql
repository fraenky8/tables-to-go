...
CREATE TABLE real_table (
    ...
    real_nn_check real NOT NULL CHECK ( real_nn_check > 0 ),
    ...
    real_nn_unique_check real NOT NULL UNIQUE CHECK ( real_nn_unique_check > 0 ),

    real_unique real UNIQUE,
    real_unique_check real UNIQUE CHECK ( real_unique_check > 0 ),
    ...
    real_check_ref real CHECK ( real_check_ref > 0 ) REFERENCES real_ref(real_ref),
    real_check_def_const real CHECK ( real_check_def_const > 0 ) DEFAULT 42,
    real_check_def_func real CHECK ( real_check_def_func > 0 ) DEFAULT (pi()),

    real_ref real REFERENCES real_ref(real_ref),
    real_ref_unique_check real UNIQUE CHECK ( real_ref_unique_check > 0 ) REFERENCES real_ref(real_ref),

    real_def_const real DEFAULT 42,
    real_def_const_unique_check real UNIQUE CHECK ( real_def_const_unique_check > 0 ) DEFAULT 42,

    real_def_func real DEFAULT (pi()),
    real_def_func_unique_check real UNIQUE CHECK ( real_def_func_unique_check > 0 ) DEFAULT (pi())
);
