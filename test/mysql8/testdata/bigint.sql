...
CREATE TABLE bigint_table (
    ...
    bigint_nn_check bigint NOT NULL CHECK ( bigint_nn_check > 0 ),

    bigint_unique bigint UNIQUE,
    bigint_unique_check bigint UNIQUE CHECK ( bigint_unique_check > 0 ),
    ...
    bigint_check bigint CHECK ( bigint_check > 0 ),
    bigint_check_ref bigint CHECK ( bigint_check_ref > 0 ) REFERENCES bigint_ref(bigint_ref),
    bigint_check_def_const bigint CHECK ( bigint_check_def_const > 0 ) DEFAULT 42,
    bigint_check_def_func bigint CHECK ( bigint_check_def_func > 0 ) DEFAULT (pi()),

    bigint_ref bigint REFERENCES bigint_ref(bigint_ref),
    bigint_ref_unique_check bigint UNIQUE CHECK ( bigint_ref_unique_check > 0 ) REFERENCES bigint_ref(bigint_ref),

    bigint_def_const bigint DEFAULT 42,
    bigint_def_const_unique_check bigint UNIQUE CHECK ( bigint_def_const_unique_check > 0 ) DEFAULT 42,

    bigint_def_func bigint DEFAULT (pi()),
    bigint_def_func_unique_check bigint UNIQUE CHECK ( bigint_def_func_unique_check > 0 ) DEFAULT (pi())
);
