...
(
    ...
    float_nn_check               float NOT NULL,
    float_nn_ref                 float NOT NULL REFERENCES float_ref (float_ref),
    float_nn_def_const           float NOT NULL DEFAULT 42,
    float_nn_def_func            float NOT NULL DEFAULT 42,
    float_nn_unique_check        float NOT NULL UNIQUE,

    float_unique                 float UNIQUE,
    float_unique_check           float UNIQUE,
    float_unique_ref             float UNIQUE REFERENCES float_ref (float_ref),
    float_unique_def_const       float UNIQUE   DEFAULT 42,
    float_unique_def_func        float UNIQUE   DEFAULT 42,

    float_check                  float,
    float_check_ref              float REFERENCES float_ref (float_ref),
    float_check_def_const        float          DEFAULT 42,
    float_check_def_func         float          DEFAULT 42,

    float_ref                    float REFERENCES float_ref (float_ref),
    float_ref_unique_check       float UNIQUE REFERENCES float_ref (float_ref),

    float_def_const              float          DEFAULT 42,
    float_def_const_unique_check float UNIQUE   DEFAULT 42,

    float_def_func               float          DEFAULT 42,
    float_def_func_unique_check  float UNIQUE   DEFAULT 42
);
...
(
    float_pk_def_func float PRIMARY KEY DEFAULT 42
);
...
(
    float_nn_unique_check_pk float PRIMARY KEY NOT NULL UNIQUE
);

DROP TABLE IF EXISTS float_nn_unique_check_pk_ref;
CREATE TABLE float_nn_unique_check_pk_ref
(
    float_nn_unique_check_pk_ref float PRIMARY KEY NOT NULL UNIQUE REFERENCES float_ref (float_ref)
);
