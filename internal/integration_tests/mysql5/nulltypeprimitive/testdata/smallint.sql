...
(
    ...
    smallint_nn_check               smallint NOT NULL,

    smallint_unique                 smallint UNIQUE,
    smallint_unique_check           smallint UNIQUE,
    ...
    smallint_unique_def_func        smallint UNIQUE DEFAULT 42,

    smallint_check                  smallint,
    smallint_check_ref              smallint REFERENCES smallint_ref (smallint_ref),
    smallint_check_def_const        smallint        DEFAULT 42,
    smallint_check_def_func         smallint        DEFAULT 42,

    smallint_ref                    smallint REFERENCES smallint_ref (smallint_ref),
    smallint_ref_unique_check       smallint UNIQUE REFERENCES smallint_ref (smallint_ref),

    smallint_def_const              smallint        DEFAULT 42,
    smallint_def_const_unique_check smallint UNIQUE DEFAULT 42,

    smallint_def_func               smallint        DEFAULT 42,
    smallint_def_func_unique_check  smallint UNIQUE DEFAULT 42
);
...
(
    smallint_pk_def_func smallint PRIMARY KEY DEFAULT 42
);
...
(
    smallint_nn_unique_check_pk smallint PRIMARY KEY NOT NULL UNIQUE
);

DROP TABLE IF EXISTS smallint_nn_unique_check_pk_ref;
CREATE TABLE smallint_nn_unique_check_pk_ref
(
    smallint_nn_unique_check_pk_ref smallint PRIMARY KEY NOT NULL UNIQUE REFERENCES smallint_ref (smallint_ref)
);
