...
CREATE TABLE float_table (
    ...
    float_nn_check float NOT NULL CHECK ( float_nn_check > 0 ),
    ...
    float_nn_unique_check float NOT NULL UNIQUE CHECK ( float_nn_unique_check > 0 ),

    float_unique float UNIQUE,
    float_unique_check float UNIQUE CHECK ( float_unique_check > 0 ),
    ...
    float_check float CHECK ( float_check > 0 ),
    float_check_ref float CHECK ( float_check_ref > 0 ) REFERENCES float_ref(float_ref),
    float_check_def_const float CHECK ( float_check_def_const > 0 ) DEFAULT 42,
    float_check_def_func float CHECK ( float_check_def_func > 0 ) DEFAULT (pi()),

    float_ref float REFERENCES float_ref(float_ref),
    float_ref_unique_check float UNIQUE CHECK ( float_ref_unique_check > 0 ) REFERENCES float_ref(float_ref),

    float_def_const float DEFAULT 42,
    float_def_const_unique_check float UNIQUE CHECK ( float_def_const_unique_check > 0 ) DEFAULT 42,

    float_def_func float DEFAULT (pi()),
    float_def_func_unique_check float UNIQUE CHECK ( float_def_func_unique_check > 0 ) DEFAULT (pi())
);
