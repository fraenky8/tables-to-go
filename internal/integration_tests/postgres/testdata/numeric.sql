...
CREATE TABLE numeric (
    ...
    numeric_nn_unique_check numeric NOT NULL UNIQUE CHECK ( numeric > 0 ),

    numeric_unique numeric UNIQUE,
    numeric_unique_check numeric UNIQUE CHECK ( numeric > 0 ),
    ...
    numeric_check numeric CHECK ( numeric > 0 ),
    numeric_check_ref numeric CHECK ( numeric > 0 ) REFERENCES numeric_ref(numeric_ref),
    numeric_check_def_const numeric CHECK ( numeric > 0 ) DEFAULT 42,
    numeric_check_def_func numeric CHECK ( numeric > 0 ) DEFAULT pi(),

    numeric_ref numeric REFERENCES numeric_ref(numeric_ref),
    numeric_ref_def_const numeric REFERENCES numeric_ref(numeric_ref) DEFAULT 42,
    numeric_ref_def_func numeric REFERENCES numeric_ref(numeric_ref) DEFAULT pi(),
    numeric_ref_unique_check numeric UNIQUE CHECK ( numeric > 0 ) REFERENCES numeric_ref(numeric_ref),

    numeric_def_const numeric DEFAULT 42,
    numeric_def_const_unique_check numeric UNIQUE CHECK ( numeric > 0 )DEFAULT 42,

    numeric_def_func numeric DEFAULT pi(),
    numeric_def_func_unique_check numeric UNIQUE CHECK ( numeric > 0 ) DEFAULT pi()
);
